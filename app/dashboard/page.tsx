            ...
            <details key={key.id} style={{ borderTop: i === 0 ? "none" : "1px solid var(--color-divider)" }}>
              ...
              <div className="px-4 pb-5 pt-4" style={{ borderTop: "1px solid var(--color-divider)" }}>
                ...
                <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-[13.5px]">
                  <div style={kicker}>Identity</div><div className="mono">{key.name} &lt;{key.email}&gt;</div>
                  <div style={kicker}>Algorithm</div><div>{key.algorithm}</div>
                  <div style={kicker}>Expires</div><div>{key.expiresAt ? key.expiresAt.toLocaleDateString() : "Never"}</div>
                  {key.revokedAt && <><div style={kicker}>Revoked</div><div>{key.revokedAt.toLocaleDateString()}</div></>}
                </div>
